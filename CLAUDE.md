...
# Run a single test
uv run pytest tests/test_prd.py::TestGeneratePlan::test_returns_task_id_on_success -v
```
...
- **Plan generation** (`ralpher/plan/plan.py`): Generates a Project Plan from a user prompt via the `claude` CLI subprocess. Stores output in `.ralpher/projects/{task_id}/PLAN.md`.
...
- **Models** (`ralpher/models.py`): Pydantic models — `ProjectPlan` (project, description, tasks), `Task` (id, title, description, acceptance_criteria, priority, passes, notes), `Status`, and `Question`/`Questions`.
- **Loop execution** (`ralpher/loop/`): Runs `claude` iteratively up to a max iteration count (default 30). Tracks state per project under `.ralpher/projects/`. Manages git branches (`ralph/{task_id_suffix}`).
- **Utils**:
...
- Project directory structure: `.ralpher/projects/{task_id}/` contains PLAN.md, plan.json, PROMPT.md, progress.md, current_task.json, and logs/.
